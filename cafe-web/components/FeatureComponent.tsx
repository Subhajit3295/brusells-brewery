...
const FeatureComponent = (props: Props) => {
  return (
    <div className={`h-1/2 w-full flex flex-col justify-center gap-3 flex-1 md:h-full ${props.className}`}>
      <h2 className={`text-2xl md:text-3xl lg:text-5xl font-bold ${playfairDisplay.className}`}>{props.heading}</h2>
      <p className={`md:text-lg lg:text-xl text-[#d2b7a1] my-5 ${merriweather.className}`}>
        {props.paragraph}
      </p>
